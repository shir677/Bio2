...
def process_arguments():
    ...
    parser.add_argument('-a', '--algorithem', type=str,
                        help='algorithem to run, by defult run the main algorithem, enter 2 for darwin 3 for lamark')
    parser.add_argument('-e', '--enc', type=str, help='path to ciphper text')
    parser.add_argument('-f', '--freq', type=str, help='path to Letter_Freq')
    parser.add_argument('-c', '--common', type=str, help='path to common word text')
    parser.add_argument('-r', '--result', type=str, help='path to translatation text')
    parser.add_argument('-d', '--dict', type=str, help='path to translatation dict')
...
if __name__ == "__main__":
    ...
    if args.algorithem:
        run = int(args.algorithem)
    if args.enc:
        CIPHER_TEXT_FILE = args.enc
    if args.freq:
        FREQ_FILE = args.freq
    if args.common:
        COMMON_WORD = args.common
    if args.result:
        PERMUTATION_FILE = args.result
    if args.dict:
        PLAIN_TEXT_FILE = args.dict
    ...
    print(f"Total Steps: {str(total_steps)}")